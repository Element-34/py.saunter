...
```
4. The junit report is in `logs/latest.xml`. 
Even though each run produces its own result, a copy is made to `latest.xml`. 
A clone of which is stored in the job-specific directory.
...
Config files
------------
By default, the framework will look in `support/conf` for a file called `selenium.ini`. 
There should never be a file with that name checked in to make it slightly more environment-proof. 
Instead, create a symlink or copy a file in place and rename it.
